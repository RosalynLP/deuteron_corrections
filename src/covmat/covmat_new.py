...
def make_covmat(label, proton, deuteron, expcov):
    
    expcovmat = expcov.values

    proton_theory = proton["theory_central"]
    D = deuteron["data_central"]
    deuteron_theory = deuteron["theory_central"]
    
    
    # Calculating errors on T_fp1 by taking standard deviation
    proton_reps = proton.loc[:, proton.columns.str.contains("rep")]
    deuteron_reps = deuteron.loc[:, deuteron.columns.str.contains("rep")]
    nprotonrep = len(proton_reps.values.T)
    ndeuteronrep = len(deuteron_reps.values.T)

    proton_repeat = np.tile(proton_theory.values, (ndeuteronrep,1)).T
    deltas = deuteron_reps.values - proton_repeat

    deuteron_repeat = np.tile(deuteron_theory.values, (ndeuteronrep,1)).T
    deltasprime =  deuteron_reps.values - deuteron_repeat

    shift = deuteron_theory - proton_theory
    err = np.std(deuteron_reps - proton_repeat, axis=1)
...
bcdms = pd.read_table(
    "../observables/BCDMS/output/tables/group_result_table.csv",
    dtype={"user_id": float},
    index_col=[0,1,2]
)
